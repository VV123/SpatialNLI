...
class BeamSearchDecoder(decoder.Decoder):
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with ops.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
      ...
      cell_outputs_flatbeam = tf.reshape(cell_outputs,[-1,vocab_dim])#flatten
      cell_outputs = tf.exp(cell_outputs)
      logits = tf.exp(logits)
      logits = tf.reshape( logits, [-1])
      print(logits.get_shape().as_list())
      print(meshidx.get_shape().as_list())
      logits_all = tf.sparse_to_dense(meshidx, tf.shape(cell_outputs_flatbeam) , logits, validate_indices=False)
      print('====')
