...
class BasicDecoder(decoder.Decoder):
  ...
  def __init__(self, cell, helper, initial_state, X ,output_layer=None):
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with ops.name_scope(name, "BasicDecoderStep", (time, inputs, state)):
      ...
      _, Y = tf.meshgrid(x, y)
      Y = tf.reshape(Y,[-1,1])
      idx = tf.reshape( self._X , [-1,1])
      meshidx = tf.concat([Y,idx],axis=1)
      cell_outputs = tf.exp(cell_outputs)
      logits = tf.exp(logits)
      logits = tf.reshape( logits, [-1])
      logits_all = tf.sparse_to_dense(meshidx, tf.shape(cell_outputs), logits, validate_indices=False)
      cell_outputs = tf.add(cell_outputs , logits_all)
